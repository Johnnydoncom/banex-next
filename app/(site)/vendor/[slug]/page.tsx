  ...
  try {
    ...
    if (seller) {
      return buildMetadata({
        ...
        description:
          seller.description ||
          `Order from ${seller.shop_name}${seller.location ? ` at ${seller.location}` : ""} inside Banex Mall — same-hour rider delivery or in-mall pickup, escrow protected.`,
...
export default async function VendorPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // ── Structured data (Store + Breadcrumb + catalogue ItemList) ──
  const jsonLd = [
    storeSchema({
      ...
      ratingValue: vendor.rating_average,
      reviewCount: vendor.reviews_count,
    }),
    breadcrumbSchema([
      { name: "Home", path: "/" },
      { name: "Vendors", path: "/vendors" },
      { name: vendor.shop_name, path: `/vendor/${vendor.slug}` },
    ]),
    ...(items.length
      ? [
          itemListSchema(
