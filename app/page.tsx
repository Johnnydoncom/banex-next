...
export default function Home() {
  ...
  const trending = products.slice(6, 12).length >= 3 ? products.slice(6, 12) : products.slice(0, 6)
  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-10 px-4 pb-12 pt-10 md:grid-cols-[1.1fr_1fr] md:gap-10 md:px-8 md:pb-20 md:pt-16">
          ...
          >
            ...
            <p className="mt-5 max-w-md text-base leading-relaxed text-muted-foreground">
              Shop from {sortedByProminence.length}+ vendors inside Banex Mall — phones, fashion, groceries, beauty, electronics —
              and our riders bring it to your door, same-hour, across the city.
            </p>
            ...
            <div className="mt-6 flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
              ...
              <span className="inline-flex items-center gap-1.5"><Store className="h-3.5 w-3.5 text-brand" /> {sortedByProminence.length} mall tenants</span>
              <span className="inline-flex items-center gap-1.5"><MapPin className="h-3.5 w-3.5 text-brand" /> <Link href="/mall-map" className="hover:text-brand">In-mall pickup available</Link></span>
            </div>
          ...
          >
            ...
            <div className="relative h-full overflow-hidden rounded-3xl border border-border bg-card shadow-brand">
              ...
              <div className="absolute inset-x-4 bottom-4 flex items-center justify-between rounded-2xl bg-card/95 px-4 py-3 backdrop-blur">
                <div>
                  <p className="text-[10px] font-semibold uppercase tracking-widest text-brand-deep">Live from the mall</p>
                  <p className="font-display text-lg font-bold text-foreground">{sortedByProminence.filter((v) => v.openNow).length} shops open now</p>
                </div>
                <Link href="/mall-map" className="inline-flex items-center gap-1 rounded-full bg-gradient-brand px-3 py-1.5 text-[11px] font-semibold text-primary-foreground">
            ...
            return (
              ...
              >
                ...
                <p className="mt-3 flex items-center gap-1.5 font-display text-sm font-semibold text-foreground">
                  <Icon className="h-3.5 w-3.5 text-brand" /> {c.name}
                </p>
                <p className="mt-0.5 line-clamp-1 text-[11px] text-muted-foreground">{c.blurb}</p>
              </Link>
            )
          })}
        </div>
      ...
      <section className="mx-auto max-w-7xl px-4 pb-14 md:px-8">
        <div className="flex items-end justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-widest text-brand-deep">Editor's picks</p>
          ...
          <div className="relative grid gap-8 md:grid-cols-2 md:items-center">
            <div>
              ...
              <p className="mt-4 max-w-md text-sm opacity-90">
                List anything from a phone to a property — set your price, get verified, and get paid securely. Zero setup fees.
              </p>
            </div>
